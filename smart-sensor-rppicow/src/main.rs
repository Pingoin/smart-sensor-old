...
use wall_clock_time::{get_date_time,  sntpc, update_clock_time};
...
pub async fn ntp_task(stack: &'static Stack<cyw43::NetDriver<'static>>, server: Ipv4Address) {
    ...
    loop {
        let result = ntp_client.update_time().await;
        if let Some(timestamp) = result {
            update_clock_time(timestamp.time);
            println!(
                "Clock updated\n Offset = {}\n Roundtrip = {}",
                timestamp.offset, timestamp.roundtime
            );
        }
...
async fn main(spawner: Spawner) {
    ...
    wall_clock_time::init_clock_time();
